...
function setGenderPopulation(dataReader, sampler, query, data, callback) {
  ...
  async.parallel([(maleCallback) => {
    ...
    dataReader.byDistrictAndGender(maleParams, (err, result) => {
      if (err) {
        logger.error('Error retreiving population data for males', err.message, err.stack);
        return maleCallback(err);
      }
      data.gender.male.population = result;
      data.gender.male.sampleSize = sampler.population({ result });
      maleCallback(null);
    });
  }, (femaleCallback) => {
    const femaleParams = {
    ...
    dataReader.byDistrictAndGender(femaleParams, (err, result) => {
      if (err) {
        logger.error('Error retreiving population data for females', err.message, err.stack);
        return femaleCallback(err);
      }
      data.gender.female.population = result;
      data.gender.female.sampleSize = sampler.population({ result });
      femaleCallback(null);
    });
  }], (err) => {
    if (err) {
